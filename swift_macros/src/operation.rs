...
pub(crate) fn process_operation(input: String) -> TokenStream {
    ...
    let bundle = generate_bundle(bundle_ident.clone(), op_ident.clone(), model_ident.clone(), &reads, &writes);

    let operation_body: TokenStream = any_pat.replace_all(&*input, |caps: &Captures| {
        format!("_swift_engine_resource_{}", &caps[2])
    }).to_string().parse().expect("could not parse after replacing");
    let op = generate_operation(op_ident, output_ident.clone(), model_ident.clone(), extras_module_ident.clone(), &reads, &writes, operation_body);
...
fn generate_bundle(ident: Ident, op_ident: Ident, model_ident: Ident, reads: &Vec<&str>, writes: &Vec<&str>) -> TokenStream {
    let read_idents = reads.iter().map(|r| format_ident!("{r}")).collect::<Vec<_>>();
    let write_idents = writes.iter().map(|r| format_ident!("{r}")).collect::<Vec<_>>();

    let child_idents = reads.iter().map(|r| format_ident!("{r}_child")).collect::<Vec<_>>();
    let write_node_idents = writes.iter().map(|r| format_ident!("{r}_write_node")).collect::<Vec<_>>();
...
fn generate_operation(ident: Ident, output_ident: Ident, model_ident: Ident, extras_module_ident: Ident, reads: &Vec<&str>, writes: &Vec<&str>, body: TokenStream) -> TokenStream {
    let read_idents = reads.iter().map(|r| format_ident!("{r}")).collect::<Vec<_>>();
    let write_idents = writes.iter().map(|r| format_ident!("{r}")).collect::<Vec<_>>();

    let read_only_idents = read_idents.iter().filter(|i| !write_idents.contains(i)).collect::<Vec<_>>();
    let write_only_idents = write_idents.iter().filter(|i| !read_idents.contains(i)).collect::<Vec<_>>();
    let read_write_idents = read_idents.iter().filter(|i| write_idents.contains(i)).collect::<Vec<_>>();

    let read_only_resource_idents = read_only_idents.iter().map(|i| format_ident!("_swift_engine_resource_{i}")).collect::<Vec<_>>();
    let write_only_resource_idents = write_only_idents.iter().map(|i| format_ident!("_swift_engine_resource_{i}")).collect::<Vec<_>>();
    let read_write_resource_idents = read_write_idents.iter().map(|i| format_ident!("_swift_engine_resource_{i}")).collect::<Vec<_>>();

    let mut all_write_resource_idents =  write_idents.iter().map(|i| format_ident!("_swift_engine_resource_{i}")).collect::<Vec<_>>();

    let child_idents = reads.iter().map(|r| format_ident!("{r}_child")).collect::<Vec<_>>();

    let read_only_child_idents = read_only_idents.iter().map(|r| format_ident!("{r}_child")).collect::<Vec<_>>();
    let read_write_child_idents = read_write_idents.iter().map(|r| format_ident!("{r}_child")).collect::<Vec<_>>();

    let child_resource_type_tag_idents = reads.iter().map(|r| format_ident!("{r}ResourceTypeTag"));
    let write_only_resource_type_tag_idents = write_only_idents.iter().map(|i| format_ident!("{i}ResourceTypeTag"));
    let all_write_resource_type_tag_idents = write_idents.iter().map(|i| format_ident!("{i}ResourceTypeTag"));
    ...
    quote! {
        ...
        impl #ident {
            async fn run(&mut self, history: &<#model_ident as swift::Model>::History) {
                ...
                self.output = Some(#output_ident {
                    #(#write_idents: #all_write_resource_idents.clone(),)*
                });

                let hash = self.history_hash();
                #(history.#write_idents.insert(hash, #all_write_resource_idents);)*
            }
        ...
        #(
            ...
            impl swift::operation::Operation<#model_ident, crate::#extras_module_ident::#all_write_resource_type_tag_idents> for swift::reexports::tokio::sync::RwLock<#ident> {
                async fn run(&self, history: &<#model_ident as swift::Model>::History) -> swift::reexports::tokio::sync::RwLockReadGuard<<crate::#extras_module_ident::#all_write_resource_type_tag_idents as swift::resource::ResourceTypeTag>::ResourceType> {
                    ...
                    return swift::reexports::tokio::sync::RwLockReadGuard::map(self.read().await, |o| &o.output.as_ref().unwrap().#write_idents);
                }
